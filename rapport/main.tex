...
\author[label2]{Encadré par : Pr. Oumayma OUEDRHIRI}
\author[label2]{Pr. Hiba TABBAA}
\author[label2]{Pr. Mohamed LACHGAR}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.85\textwidth]{architecture-microservices.png}
        \caption{Architecture des microservices AquaWatch - Flux de données entre les composants}
...
\subsubsection{Service Discovery avec Eureka}

L'architecture distribuée d'AquaWatch nécessite un mécanisme dynamique pour la découverte des services. Nous utilisons \textbf{Netflix Eureka} comme registre de services centralisé.

\textbf{Rôle d'Eureka :}
\begin{itemize}
    \item \textbf{Enregistrement} : Chaque microservice (Node.js ou Python) s'enregistre au démarrage auprès du serveur Eureka.
    \item \textbf{Découverte} : Les services (ex: API-SIG) interrogent Eureka pour obtenir l'adresse IP et le port des autres services (ex: Capteurs).
    \item \textbf{High Availability} : Eureka maintient une liste à jour des instances saines via des heartbeat réguliers.
\end{itemize}

La figure \ref{fig:eureka-dashboard} montre le tableau de bord Eureka avec l'ensemble des 5 microservices enregistrés et actifs ("UP").

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Eureka1.png}
    \caption{Tableau de bord Eureka - Vue d'ensemble des services enregistrés}
    \label{fig:eureka-dashboard}
\end{figure}

L'historique des enregistrements (Figure \ref{fig:eureka-history}) permet de tracer les démarrages et arrêts des instances, facilitant le débogage et la surveillance du cluster.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{Eureka2.png}
    \caption{Historique des enregistrements et renouvellements de baux dans Eureka}
    \label{fig:eureka-history}
\end{figure}

\subsubsection{Pipeline CI/CD avec Jenkins}

Le projet intègre un pipeline d'intégration continue (CI) et de déploiement continu (CD) basé sur \textbf{Jenkins}, permettant l'automatisation du build, des tests et du déploiement des microservices.

\textbf{Rôle du Pipeline CI/CD :}

\begin{itemize}
    \item \textbf{Automatisation} : Un seul clic déclenche le clone, le build et le déploiement de tous les services
    \item \textbf{Reproductibilité} : Même processus garanti sur tous les environnements
    \item \textbf{Traçabilité} : Historique complet des builds avec logs et durées
    \item \textbf{Détection d'erreurs} : Tests de santé automatiques après déploiement
\end{itemize}

\textbf{Stages du Pipeline :}
...
\hline
\textbf{Stage} & \textbf{Durée} & \textbf{Action} \\
\hline
Checkout & ~900ms & Récupération du code source \\
\hline
Environment Setup & ~400ms & Création de la configuration CI isolée \\
\hline
Infrastructure & ~4s & Démarrage TimeScaleDB, MQTT, Eureka \\
\hline
Capteurs & ~1m 30s & Build et démarrage du service Capteurs \\
\hline
Satellite & ~1m 40s & Build et démarrage du service Satellite \\
\hline
STModel & ~1m 20s & Build et démarrage du modèle IA \\
\hline
Alertes & ~90ms & Build et démarrage du service Alertes \\
\hline
API-SIG & ~70ms & Build et démarrage de l'API Node.js \\
\hline
Web & ~80ms & Build et démarrage du Frontend Nginx \\
\hline
Health Checks & ~300ms & Test de santé sur ports isolés \\
\hline
\end{tabular}
\end{table}

\textbf{Extrait du Jenkinsfile (Configuration Isolée) :}

\begin{lstlisting}[language=Groovy, caption=Pipeline Jenkins avec isolation des environnements]
pipeline {
    agent any
    environment {
        // Isolation des conteneurs pour éviter les conflits
    ...
    stages {
        stage('Environment Setup') {
            steps {
                // Génération dynamique de la configuration de test
                writeFile file: 'docker-compose.ci.yml', text: """
version: '3.8'
services:
  eureka: { container_name: ci-eureka-${BUILD_NUMBER}, ports: ["18761:8761"] }
  api-sig: { ports: ["13000:3000"] }
  // ... autres services avec ports décalés
"""
            }
        }
        stage('Infrastructure') {
            steps {
                sh "${DOCKER_COMPOSE_CMD} up -d timescaledb mqtt eurkea"
            }
        }
        // Stages de déploiement individuels...
        stage('Health Checks') {
            steps {
                // Vérification sur les ports de test (13000 vs 3000)
                sh 'curl -f http://host.docker.internal:13000/health'
            }
        }
    }
    post {
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jenkins_pipeline_success.png}
    \caption{Vue des stages du pipeline Jenkins - Exécution granulaire avec succès}
...
\textbf{Utilisation bidirectionnelle :} La matrice spatiale 4×4 est utilisée \textbf{à la fois pendant l'entraînement} (pour créer les séquences d'apprentissage avec positions fixes) \textbf{et pendant la prédiction} (pour générer les prévisions 24h en conservant la même organisation spatiale).
...
\subsubsection{Convolutions Spatiales et Relations de Voisinage}

Le ConvLSTM utilise un \textbf{noyau de convolution 3×3} (kernel\_size=3) qui analyse une fenêtre de 9 positions autour de chaque zone. Cette opération permet de capturer les relations spatiales entre zones adjacentes.

\textbf{Exemple concret :} Pour prédire la qualité à Rabat-Centre [0,0], le modèle examine automatiquement :
\begin{itemize}
    \item Hay-Riad [0,1] : voisin horizontal (est)
    \item Salé-Nord [1,0] : voisin vertical (sud)
    \item Agdal [1,1] : voisin diagonal (sud-est)
    \item Et les autres zones dans la fenêtre 3×3
\end{itemize}

Cette architecture permet au modèle d'apprendre que \textit{"si Bouregreg (rivière) a une turbidité élevée, cela peut affecter Rabat-Centre et Salé-Nord"} ou que \textit{"les zones côtières (Côte-Océan) ont des patterns de température différents des zones intérieures"}.
...
\subsubsection{Processus d'Entraînement par Fenêtre Glissante}

L'entraînement du modèle utilise une technique de \textbf{fenêtre glissante} (sliding window) sur les données historiques. Pour chaque zone, le système crée des milliers de séquences d'apprentissage :

\textbf{Mécanisme de la fenêtre glissante :}
\begin{enumerate}
    \item \textbf{Séquence 1} : Les 12 premières heures (H0 → H11) servent d'entrée, le modèle prédit H12
    \item \textbf{Séquence 2} : La fenêtre glisse d'une heure (H1 → H12), le modèle prédit H13
    \item \textbf{Séquence 3} : Nouvelle glissade (H2 → H13), le modèle prédit H14
    \item Et ainsi de suite jusqu'à épuisement des données historiques
\end{enumerate}

\textbf{Exemple numérique :} À partir de 23,368 enregistrements collectés sur 7 jours, le système génère plusieurs milliers de séquences d'entraînement. Pour chaque séquence :
\begin{itemize}
    \item Le modèle reçoit 12 matrices 4×4 (une par heure) avec les valeurs normalisées de pH, turbidité, température
    \item Il prédit les valeurs de l'heure suivante (H13)
    \item Il compare sa prédiction avec la \textbf{valeur réelle} de H13 dans la base de données
    \item Il calcule l'erreur (MSE) et ajuste ses poids via rétropropagation
\end{itemize}

Ce processus itératif permet au modèle d'apprendre les patterns temporels (variations jour/nuit) et spatiaux (propagation de pollution entre zones voisines).
