...
\begin{enumerate}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.85\textwidth]{images/architecture-microservices.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/Eureka1.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/Eureka2.png}
...
\hline
Checkout & ~4s & Clone le code depuis GitHub (shallow clone) \\
\hline
Environment Setup & ~2s & Vérifie Docker et Docker Compose \\
\hline
Build Services & ~20s & Construit les images Docker en parallèle \\
\hline
Start Services & ~35s & Démarre les 11 microservices \\
\hline
Health Checks & ~1s & Teste l'API et l'interface web \\
\hline
\end{tabular}
\end{table}
...
\begin{lstlisting}[language=Groovy, caption=Pipeline Jenkins simplifié]
pipeline {
    agent any
    stages {
        stage('Checkout') {
            steps {
                checkout([$class: 'GitSCM',
                    branches: [[name: '*/main']],
                    userRemoteConfigs: [[
                        url: 'https://github.com/ghayted/AQUA.git'
                    ]]
                ])
            }
        }
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{jenkins-pipeline.png}
    \caption{Vue des stages du pipeline Jenkins - Build réussi avec tous les services déployés}
